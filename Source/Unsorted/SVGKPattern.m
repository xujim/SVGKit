...
+ (SVGKPattern*)patternWithNSColor:(NSColor*)color
{
	if ([color respondsToSelector:@selector(CGColor)]) {
		...
		if ([[color colorSpace] colorSpaceModel] == NSPatternColorSpaceModel) {
			...
			SVGKitLogWarn(@"The color %@ is not a pattern color. Attempting to convert to CGColor", [color description]);
			CGColorSpaceRef spaceRef = [[color colorSpace] CGColorSpace];
			if (!spaceRef) {
				SVGKitLogError(@"Could not get CGColorSpace from the color %@", [color description]);
				return nil;
			}
			...
			@catch (NSException *exception) {
				SVGKitLogError(@"Color %@ does not have components", [color description]);
				return nil;
			}
